-- Create missing ML and performance tables identified in code review
-- Fixed version with proper constraint syntax
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE constraint_name = 'fk_opportunities_listing' 
                   AND table_name = 'opportunities') THEN
        ALTER TABLE public.opportunities ADD CONSTRAINT fk_opportunities_listing 
            FOREIGN KEY (listing_id) REFERENCES public.public_listings(id) ON DELETE SET NULL;
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Service role can manage field provenance" ON public.field_provenance;
CREATE POLICY "Service role can manage field provenance" ON public.field_provenance
    FOR ALL USING (auth.role() = 'service_role'::text);

DROP POLICY IF EXISTS "Authenticated users can read field provenance" ON public.field_provenance;
...
DROP POLICY IF EXISTS "Service role can manage model metrics" ON public.model_performance_metrics;
CREATE POLICY "Service role can manage model metrics" ON public.model_performance_metrics
    FOR ALL USING (auth.role() = 'service_role'::text);

DROP POLICY IF EXISTS "Authenticated users can read model metrics" ON public.model_performance_metrics;
CREATE POLICY "Authenticated users can read model metrics" ON public.model_performance_metrics
    FOR SELECT USING (auth.role() = 'authenticated'::text);

DROP POLICY IF EXISTS "Service role can manage ML models" ON public.ml_models;
CREATE POLICY "Service role can manage ML models" ON public.ml_models
    FOR ALL USING (auth.role() = 'service_role'::text);

DROP POLICY IF EXISTS "Authenticated users can read ML models" ON public.ml_models;
CREATE POLICY "Authenticated users can read ML models" ON public.ml_models
    FOR SELECT USING (auth.role() = 'authenticated'::text);

-- Add triggers for timestamp management (drop first if exists)
DROP TRIGGER IF EXISTS update_field_provenance_updated_at ON public.field_provenance;
