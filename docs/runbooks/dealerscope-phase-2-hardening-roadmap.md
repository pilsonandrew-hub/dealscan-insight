...
- Current posture before this fix:
  - Auth: `/api/ingest/apify` requires `X-Apify-Webhook-Secret`, accepts the current secret and optional previous secret, rejects stale payloads when `APIFY_WEBHOOK_MAX_AGE_SECONDS` is set, and ignores recent run-id replays using `webhook_log` in [webapp/routers/ingest.py](/Users/andrewpilson/.openclaw/workspace/projects/dealerscope/webapp/routers/ingest.py).
  - Rate limiting: the old middleware treated ingest like generic API traffic, trusted proxy headers too broadly, used the last `X-Forwarded-For` hop, and failed open whenever Redis init or Redis operations failed in [webapp/middleware/rate_limit.py](/Users/andrewpilson/.openclaw/workspace/projects/dealerscope/webapp/middleware/rate_limit.py).
  - Fail-open gap: a Redis outage or spoofed proxy header could weaken the ingest boundary even when the shared-secret check stayed intact.
- Repo-backed hardening now implemented:
  - `/api/ingest/apify` has its own limit profile in [webapp/middleware/rate_limit.py](/Users/andrewpilson/.openclaw/workspace/projects/dealerscope/webapp/middleware/rate_limit.py): `rate_limit_ingest_requests=10` per `rate_limit_ingest_window_seconds=60` by default, tighter than generic API traffic.
  - Proxy headers are ignored unless `rate_limit_trust_proxy_headers=true` and the direct peer IP is inside `rate_limit_trusted_proxy_cidrs`. When trusted proxies are configured, `X-Forwarded-For` resolves to the leftmost untrusted hop, not the last hop.
...
- Exact deliverable: make the GitHub-based health/pager runner use a known-good live database connection path and record the exact repo secret shape or retrieval method that works from Actions. The repo-backed decision is to stop exporting `DATABASE_URL` in that workflow and instead prefer a Supabase direct path: `SUPABASE_DB_URL` / `SUPABASE_DATABASE_URL` / `SUPABASE_DIRECT_DB_URL`, or a derived direct DSN from `SUPABASE_DB_PASSWORD` plus `SUPABASE_PROJECT_ID`. The runner must complete the health check against the intended live database, not merely fire the Telegram send path.
