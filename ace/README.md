...
- CLI commands: `init`, `bootstrap`, `intake`, `list`, `show`, `sweep`, `briefing`, `cycle`, `cycle-status`, `supervisor-run`, `supervisor-status`, `add-evidence`, `add-obligation`, `add-contradiction`, `resolve-obligation`, `resolve-contradiction`, `approve`, `block`, `done`, `resolve`, `drop`
