...
CREATE INDEX idx_opportunities_user_profit ON public.opportunities(user_id, potential_profit DESC) WHERE is_active = true;
CREATE INDEX idx_public_listings_active_created ON public.public_listings(is_active, created_at DESC);
CREATE INDEX idx_public_listings_site_active ON public.public_listings(source_site, is_active) WHERE is_active = true;
CREATE INDEX idx_dealer_sales_user_date ON public.dealer_sales(user_id, sale_date DESC);
CREATE INDEX idx_market_prices_make_model_year ON public.market_prices(make, model, year, state);
CREATE INDEX idx_user_alerts_user_dismissed ON public.user_alerts(user_id, dismissed, created_at DESC);
