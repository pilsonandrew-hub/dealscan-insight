...
def test_safe_truth_audit_market_data_quality_marks_pricing_substrate_unavailable_on_fetch_error(monkeypatch):
    columns = {
        "opportunities": {
            "created_at",
            "dos_score",
    ...
    def fake_columns(_base, _key, table):
        if table in {"market_prices", "dealer_sales"}:
            raise RuntimeError(f"{table} read failed")
        return columns[table]

    monkeypatch.setattr(inspection, "_available_columns", fake_columns)
    monkeypatch.setattr(
        inspection,
        "_recent_rows",
        lambda _base, _key, table, _select, _order, _limit: rows[table],
    ...
    assert "pricing_substrate" in quality["unavailable_dimensions"]
    assert "pricing_substrate_unavailable" in quality["degraded_reasons"]
    assert "market_prices_unusable" not in quality["degraded_reasons"]
    assert "dealer_sales_sparse" not in quality["degraded_reasons"]


def test_safe_truth_audit_market_data_quality_uses_recent_dealer_sales_only(monkeypatch):
    columns = {
        "opportunities": {
            "created_at",
            "dos_score",
    ...
    rows = {
        ...
        "market_prices": [],
        "dealer_sales": [
            {"id": "old-sale-1", "sale_price": 22000, "sale_date": "2020-01-01T00:00:00+00:00"},
            {"id": "old-sale-2", "sale_price": 23000, "sale_date": "2020-01-02T00:00:00+00:00"},
        ],
    }
