...
def _seller_recovery_candidate_evidence(row: dict[str, Any], *, source_bidder_sources: set[str]) -> dict[str, Any]:
...
def _score_seller_recovery_candidate(
    ...
    *,
    ...
    evidence = _seller_recovery_candidate_evidence(row, source_bidder_sources=source_bidder_sources)
...
def _seller_recovery_source_health_summary(
    ...
    return {
        ...
        "sources_with_failed_runs": len({
            _source_key(row.get("source_name"))
            for row in source_health_rows
            if row.get("source_name") and (_coerce_int(row.get("failed_runs")) or 0) > 0
        }),
