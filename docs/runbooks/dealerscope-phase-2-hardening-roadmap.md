...
- Exact deliverable: add a repo-owned webhook boundary proof flow that records active-secret fingerprint, previous-secret presence, deploy SHA, and red-team results after each rotation or env change. Extend preflight so it fails when the active runtime posture cannot be proven.
